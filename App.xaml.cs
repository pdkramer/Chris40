...
namespace Chris40
{
    ...
    {
        public static List<MusicInfo> SheetMusic = new List<MusicInfo>();

        public async static Task LoadSheetMusic(StorageFolder ourFolder)
        {
            SheetMusic.Clear();

            foreach (var file in await ourFolder.GetFilesAsync())
            {
                if (file.FileType.ToLower() == ".pdf")
                {
                    SheetMusic.Add(new MusicInfo { Name = file.DisplayName, FileName = file.Path });
                }
            }
        }
        ...
        protected override void OnLaunched(LaunchActivatedEventArgs e)
        {
            ...
            if (rootFrame == null)
            {
                ...
                rootFrame.ContentTransitions.Add(theme);
